...
## Windows port (Win10/Win11 x64)

A Rust port of the core functionality lives in `windows/`. It provides the same gain lock (re-applies 4×/second) and 10% floor for the Jabra Elite 85h, with a system-tray icon and a small control window. **No permissions are needed on Windows** — there is no TCC equivalent for endpoint volume control.

Out of scope for v1: Bluetooth connect/disconnect buttons (Windows manages BT audio profiles natively).
...
Tray icons are generated by `tools/make_windows_icons.py` (pure stdlib) into `windows/assets/`.

The taskbar/title-bar icon is generated by `tools/make_app_icon.rs` (pure std, no crates) into `windows/assets/icon_app.bin`, a multi-size container of pixel-art mic images. Regenerate from the repo root with:
...
CI: `.github/workflows/windows.yml` runs fmt + clippy + release build + `--list` smoke test on every push.
